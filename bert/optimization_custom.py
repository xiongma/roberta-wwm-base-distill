...
def create_optimizer_custom(loss, init_lr, num_train_steps, num_warmup_steps, use_tpu):
    """Creates an optimizer training op."""
    global_step = tf.train.get_or_create_global_step()
    ...
    # get train vars, because we fixed the teacher's vars, we don't want apply gradients to them
    train_tvars = [var for var in tvars if 'teacher' not in var.name]

    grads = tf.gradients(loss, train_tvars)

    # This is how the model was pre-trained.
    (grads, _) = tf.clip_by_global_norm(grads, clip_norm=1.0)

    train_op = optimizer.apply_gradients(
        zip(grads, train_tvars), global_step=global_step)
