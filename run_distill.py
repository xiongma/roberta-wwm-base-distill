...
flags.DEFINE_string(
    "input_file", None,
    "Input TF example files for train (can be a glob or comma separated).")
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
                     ...
                     use_one_hot_embeddings, truncation_factor):
  ...
  def model_fn(features, labels, mode, params):
    ...
    masked_lm_log_probs, masked_lm_probs = get_masked_lm_output(
         ...
         masked_lm_positions, top_k_indices, truncation_factor)

    masked_lm_loss, masked_lm_example_loss = get_masked_lm_loss(truncated_masked_lm_probs_teacher,
                                                                masked_lm_log_probs,
                                                                masked_lm_weights)

    cosin_loss, cosin_example_loss = get_cosin_loss(truncated_masked_lm_probs_teacher, masked_lm_probs, masked_lm_weights)

    total_loss = cosin_loss + masked_lm_loss
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      eval_metrics = (metric_fn, [
          masked_lm_example_loss + cosin_example_loss, masked_lm_log_probs, masked_lm_ids, masked_lm_weights])
      output_spec = tf.contrib.tpu.TPUEstimatorSpec(
          mode=mode,
          loss=total_loss,
...
def cosin(x1, x2):
    ...
    score = tf.div(x1_x2, x1_norm * x2_norm + 1e-8)

    return score

def get_cosin_loss(truncated_masked_lm_probs_teacher, top_k_log_probs_student, label_weights):
    """get cosin loss"""
    label_weights = tf.reshape(label_weights, [-1])
    prob_shape = tf.shape(top_k_log_probs_student)
    new_shape = [prob_shape[0], prob_shape[1]]

    truncated_masked_lm_probs_teacher = tf.reshape(truncated_masked_lm_probs_teacher, new_shape)

    per_example_loss = 1 - cosin(truncated_masked_lm_probs_teacher, top_k_log_probs_student)
...
def get_masked_lm_loss(truncated_masked_lm_probs_teacher, top_k_log_probs_student, label_weights):
...
def main(_):
  ...
  input_files = shuffle(input_files)
  train_input_files = input_files[:int(0.9*len(input_files))]
  eval_input_files = input_files[int(0.9*len(input_files)):]
  ...
  if FLAGS.do_train:
    tf.logging.info("***Training Input Files ***")
  ...
  if FLAGS.do_eval:
    tf.logging.info("***Eval Input Files ***")
